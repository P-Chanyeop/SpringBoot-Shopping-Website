...
public class ItemRepositoryCustomImpl implements ItemRepositoryCustom{
    ...
    // 해당 시간 이후로 등록된 상품만 조회한다.
    // 예를 들어 searchDateType 값이 "1m"일 경우 dateTime의 시간을 한 달 전으로 세팅 후
    // 최근 한달 동안 등록된 상품만 조회하도록 조건값을 반환
    ...
    // 쿼리문을 직접 작성할 때의 형태와 문법이 비슷하다.
    @Override
    public Page<Item> getAdminItemPage(ItemSearchDto itemSearchDto,
                                       Pageable pageable) {
        QueryResults<Item> results = queryFactory
                .selectFrom(QItem.item)     // 상품 데이터를 조회하기 위해 QItem의 item 지정
                .where(regDtsAfter(itemSearchDto.getSearchDateType()),  // BooleanExpression 을 반환하는 조건문을 삽입.  "," 단위로 넣어줄 경우 and 조건으로 인식
                ...
                // 조회한 리스트 및 전체 개수를 포함하는 QueryResults를 반환. 상품 데이터 리스트 조회 및 상품 데이터 전체 개수를 조회하는 2번의 쿼리문이 실행된다
                .fetchResults();
    ...
    public Page<MainItemDto> getMainItemPage(ItemSearchDto itemSearchDto,
        ...
        QueryResults<MainItemDto> results = queryFactory
                .select(
                        ...
                        // @QueryProjection을 사용하면 DTO로 바로 조회가 가능하며
